...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const educationDB = client.db.createModel("educationServers");

    return message.channel
      .send(
        "<a:loading:784127487118016573> Fetching data from the Chatwind API. Please wait"
      )
      .then(async (msg) => {
        const educationExists = await educationDB.exists(
          message.guild.id + "_education"
        );
        const educationGet = await educationDB.get(
          message.guild.id + "_education"
        );
        ...
        const errorEmbedNoAccess = new Discord.MessageEmbed()
          ...
          .setDescription(
            `Hey there! This server does not have the \`Custom Meeting Links\` feature enabled. This feature is currently locked to users on the \`Education\` plan. For a list of plans, check \`${client.config.prefix}plans\`.\n\nAlready have the \`Education\` plan? Join the official Discord server [here](https://chatwindapp.com/discord) (https://chatwindapp.com/discord) and we can help you out there.`
          )
          .setColor("RED");
        const errorEmbedNoCode = new Discord.MessageEmbed()
          .setTitle(`Custom Meeting Links`)
          .setDescription(
            `Hey there! This server does not have a custom meeting code setup! Try emailing us at \`support@chatwindapp.com\`.`
          )
          .setColor("RED");
        ...
        if (chatwindCustomCode.exists == false)
          return msg.edit("", errorEmbedNoCode);
        ...
        const codeEmbed = new Discord.MessageEmbed()
          ...
          .addField(
            "**Licensee:**",
            `${chatwindCustomCode.licensee.username}`,
            true
          )
