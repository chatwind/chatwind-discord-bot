  ...
  run: async (client, message, args) => {
    ...
    if (!argsUser)
      return message.channel.send(
        "Please specify a valid `USERNAME` to lookup."
      );
    ...
    return message.channel
      ...
      .then(async (msg) => {
        ...
        const embedInvalid = new Discord.MessageEmbed()
          ...
          .setDescription(
            `I could not find a user with the username \`${argsUser}\` in the Chatwind database.`
          )
          .setColor("ORANGE");
        ...
        if (bodyUser.message == "User not found")
          return msg.edit("", embedInvalid);

        if (bodyUser.staff == true) {
          var staff = "Yes";
        }
        if (bodyUser.staff == false) {
          var staff = "No";
        }

        if (bodyUser.premium == 0) {
          var premium = "No Premium";
        }
        if (bodyUser.premium == 2) {
          var premium = "Education";
        }
